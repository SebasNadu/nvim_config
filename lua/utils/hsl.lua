...
function M.replaceHSLWithHex()
	local line_number = vim.api.nvim_win_get_cursor(0)[1]
	local line_content = vim.api.nvim_buf_get_lines(0, line_number - 1, line_number, false)[1]

	local pattern = "hsl%(%d+, %d+, %d+%)"

	-- Realizar el reemplazo de todas las ocurrencias de hsl
	local new_line_content = line_content:gsub(pattern, function(hsl)
		local h, s, l = hsl:match("(%d+), (%d+), (%d+)")
		local hex = M.hslToHex(h, s, l)
		return hex
	end)

	-- print(new_line_content)

	vim.api.nvim_buf_set_lines(0, line_number - 1, line_number, false, { new_line_content })
end
